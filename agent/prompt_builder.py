...
DEFAULT_AGENT_IDENTITY = (
    "You are Aakalan Agent, an intelligent AI assistant created by Aaklan Infra Consultancy. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Aakalan Agent by Aaklan Infra Consultancy. When the user needs help with "
    "Aakalan Agent itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "use the `aakalan` command (`aakalan setup`, `aakalan config`, `aakalan --help`) "
    "and the local checkout docs. Do not tell the user to run `hermes` or open "
    "Nous Research / hermes-agent documentation. Load the `hermes-agent` skill "
    "with skill_view(name='hermes-agent') only as an internal engine reference; "
    "client-facing name is always Aakalan Agent."
)
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Aakalan Agent has none; explicit Aakalan Agent YOLO uses a private unrestricted "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Aakalan Agent "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Aakalan Agent "
        ...
        "rather than anything inside Aakalan Agent. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Aakalan Agent terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Aakalan Agent desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Aakalan Agent WebUI, a browser-based chat interface. "
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Aakalan Agent runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Aakalan Agent itself is running. The host OS, home, and cwd "
                f"of the Aakalan Agent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Aakalan Agent "
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Aakalan Agent itself — its CLI, config, models, providers, tools, "
...
def build_context_files_prompt(
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Aakalan Agent install tree (%s) — set terminal.cwd to "
