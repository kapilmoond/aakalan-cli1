#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Aakalan Agent icon + version metadata onto the
// built AakalanAgent.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked AakalanAgent.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-AakalanAgent.exe>
//
...
// Stamp the Aakalan Agent icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Aakalan Agent',
      FileDescription: 'Aakalan Agent',
      CompanyName: 'Aaklan Infra Consultancy',
      LegalCopyright: 'Copyright (c) 2026 Aaklan Infra Consultancy'
    }
  })

  console.log('[set-exe-identity] done — Aakalan Agent icon + identity stamped')
}
