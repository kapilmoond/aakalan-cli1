"""
Aakalan Agent Agent Uninstaller.
...
def remove_path_from_shell_configs():
    """Remove Aakalan Agent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Aakalan Agent Agent" comment and following line
                if '# Aakalan Agent Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Aakalan Agent-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Aakalan Agent-owned entries from User-scope PATH in the registry.
...
def run_gui_uninstall(args):
    ...
    print(color("│         ⚕ Aakalan Agent Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Aakalan Agent Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Aakalan Agent agent stays installed.", Colors.CYAN))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Aakalan Agent agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Aakalan Agent Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Aakalan Agent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Aakalan Agent code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Aakalan Agent PATH entries from shell configs / Windows User PATH")
    print("  • Aakalan Agent wrapper scripts and Aakalan Agent-managed node/npm/npx symlinks")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Aakalan Agent-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Aakalan Agent-set User env vars to remove")
    ...
    log_info("Removing Aakalan Agent-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Aakalan Agent-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using Aakalan Agent Agent! ⚕")
    print()
