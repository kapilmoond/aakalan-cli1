...
the app name ("Aakalan Agent"), separate from ``$HERMES_HOME``:
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Aakalan Agent"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Aakalan Agent"
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        base = Path(appdata) if appdata else (home / "AppData" / "Roaming")
        return base / "Aakalan Agent"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Aakalan Agent" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Aakalan Agent.app"),
            home / "Applications" / "Aakalan Agent.app",
        ]
        ...
        paths += [
            ...
            local_base / "Programs" / "Aakalan Agent",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Aakalan Agent")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Aakalan Agent.desktop",
        ]
    return paths
