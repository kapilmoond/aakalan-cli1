#!/usr/bin/env python3
"""
SQLite State Store for Aakalan Agent Agent.
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Aakalan Agent root {root}). "
...
def _wal_reset_repair_hint() -> str:
    ...
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Aakalan Agent-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Aakalan Agent"
    )
    ...
    *,
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Aakalan Agent needs read-write access to open the database. "
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Aakalan Agent process held the "
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Aakalan Agent, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Aakalan Agent session.

        A Aakalan Agent session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Aakalan Agent session is already bound to any Telegram DM topic.
