...
TIPS = [
    ...
    "Ctrl+Z suspends Aakalan Agent to the background — run fg in your shell to resume.",
    ...
    "hermes mcp serve runs Aakalan Agent itself as an MCP server for other agents.",
    ...
    "hermes backup creates a zip backup of your entire Aakalan Agent home directory.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Aakalan Agent tools programmatically — results stay out of context.",
    ...
    "When exiting, Aakalan Agent prints a resume command with session ID and stats.",
    ...
    # --- Gateway & Messaging ---
    "Aakalan Agent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Aakalan Agent via a local macOS server.",
    ...
    "Aakalan Agent loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "hermes acp runs Aakalan Agent as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Aakalan Agent your own.",
    ...
    "Before a gateway session reset, Aakalan Agent auto-flushes important facts to memory in the background.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Aakalan Agent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Aakalan Agent config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Aakalan Agent is working.',
    ...
    'hermes dashboard embeds the full Aakalan Agent TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'hermes import restores a full Aakalan Agent backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Aakalan Agent block commands when the tirith scanner itself errors out.',
