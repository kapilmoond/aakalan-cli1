...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Aakalan Agent triggers summarization.  Higher values delay
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Aakalan Agent Agent)",
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Aakalan Agent auth store.
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Aakalan Agent' built-in discovery
