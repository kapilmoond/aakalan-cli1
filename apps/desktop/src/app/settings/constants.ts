...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Aakalan Agent & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Aakalan Agent stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Aakalan Agent can read from one file request.',
  approvals: {
    mode: 'How Aakalan Agent handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Aakalan Agent updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
