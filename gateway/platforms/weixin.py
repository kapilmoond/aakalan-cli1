...
class WeixinAdapter(BasePlatformAdapter):
    """Native Aakalan Agent adapter for Weixin personal accounts."""
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Aakalan Agent regardless of this "
                "policy. If group delivery doesn't work, the limitation is on the iLink side, "
                "not in Aakalan Agent.",
                self.name,
                self._group_policy,
            )
        return True
