"""
Multi-provider authentication system for Aakalan Agent Agent.
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Aakalan Agent' Responses transport.
    """
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Aakalan Agent:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Aakalan Agent is waiting for the OAuth callback on {redirect_uri}")
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Aakalan Agent auth store (~/.hermes/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Aakalan Agent auth store (~/.hermes/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Aakalan Agent auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Aakalan Agent auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Aakalan Agent auth store automatically.
    """
    try:
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Aakalan Agent's own Codex token store.
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Aakalan Agent persists a Codex 429's ``reset_at`` locally and freezes the
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Aakalan Agent install sharing "
            "~/.hermes/auth.json) called POST /api/oauth/token with Aakalan Agent's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Aakalan Agent may call the "
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Aakalan Agent is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Aakalan Agent is not logged into Nous Portal.",
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Aakalan Agent' "
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Aakalan Agent auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Aakalan Agent will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Aakalan Agent will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return
    ...
    print("(Aakalan Agent creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Aakalan Agent auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Aakalan Agent creates its own local OAuth session)")
    print()
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Aakalan Agent auth store (~/.hermes/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Aakalan Agent login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Aakalan Agent login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Aakalan Agent will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hermes model` or configure an API key to use Aakalan Agent.")
        else:
