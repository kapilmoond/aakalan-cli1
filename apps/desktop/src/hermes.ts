...
// "Timed out connecting to Aakalan Agent backend" that hangs the UI (#48504).
//
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Aakalan Agent gateway connection closed',
      connectErrorMessage: 'Could not connect to Aakalan Agent gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Aakalan Agent gateway is not connected',
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('Aakalan Agent desktop bridge unavailable')
  }
