...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Aakalan Agent-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Aakalan Agent keeps databases "
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Aakalan Agent generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return True, f"other Aakalan Agent processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Aakalan Agent-managed binary is ever refreshed;
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    try:
        ...
        print(
            "  ⚠ Aakalan Agent venv links SQLite "
