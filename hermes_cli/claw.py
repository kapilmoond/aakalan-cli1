...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Aakalan Agent may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "Aakalan Agent gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Aakalan Agent")
...
def _cmd_migrate(args):
    """Run the OpenClaw → Aakalan Agent migration."""
    ...
    print(
        color(
            "│          ⚕ Aakalan Agent — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not no_backup:
        try:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Aakalan Agent home."
            )
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            "│          ⚕ Aakalan Agent — OpenClaw Cleanup                   │",
            Colors.MAGENTA,
        )
    )
    print(
