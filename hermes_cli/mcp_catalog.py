...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Aakalan Agent install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Aakalan Agent understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Aakalan Agent is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Aakalan Agent
        understands. Update Aakalan Agent to install this entry.
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    print(color(
        ...
        f"Start a new Aakalan Agent session to load its tools.",
        Colors.GREEN,
    ))
