...
export function resolveVenvHermesCommand(
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Aakalan Agent at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Aakalan Agent Python at ${python}`,
    command: python,
