 ...
 * billing-specific toast — never the generic "Aakalan Agent error" — with a smart CTA
      ...
      if (event.type === 'gateway.ready') {
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Aakalan Agent reported an error',
        ...
        const errorMessage = payload?.message || 'Aakalan Agent reported an error'
