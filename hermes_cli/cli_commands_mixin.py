...
class CLICommandsMixin:
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Aakalan Agent changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Aakalan Agent hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Aakalan Agent config/state.
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        label = _skin.get_branding("response_label", "⚕ Aakalan Agent")
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Aakalan Agent-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"lives only while this Aakalan Agent process runs — use `hermes cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Aakalan Agent keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Aakalan Agent is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Aakalan Agent is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Aakalan Agent is busy; /stop still cancels it."
    ...
    def _handle_update_command(self) -> bool:
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Aakalan Agent Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Aakalan Agent Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Aakalan Agent Agent",
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Aakalan Agent' hotword listener.
