...
def _hermes_root_path() -> Path:
    """Resolve the Aakalan Agent root dir (always the parent of any profile, never per-profile)."""
    try:
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Aakalan Agent cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Aakalan Agent credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Aakalan Agent MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Aakalan Agent MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Aakalan Agent read (see
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Aakalan Agent entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Aakalan Agent profile {info['target_profile']!r}, but the "
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Aakalan Agent-state portion starts. Returns ``None`` for
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Aakalan Agent process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Aakalan Agent state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Aakalan Agent "
