...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Aakalan Agent...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Aakalan Agent…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Aakalan Agent gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Aakalan Agent couldn't start"
